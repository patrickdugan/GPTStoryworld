    ...
    for key in ("data", "out", "resume_checkpoint"):
        ...
        if raw:
            ...
            if not p.is_absolute():
                train[key] = str((config_dir / p).resolve())
    cfg["train"] = train
    return cfg


def main() -> int:
    parser = argparse.ArgumentParser(description="Hermes wrapper for capped HRM trainer runs.")
    parser.add_argument("--config", required=True, help="JSON config path.")
    parser.add_argument("--run-id", default="", help="Optional run id override.")
    parser.add_argument("--dry-run", action="store_true", help="Write manifests/config but do not launch trainer.")
    args = parser.parse_args()

    config_path = Path(args.config).resolve()
    config = json.loads(config_path.read_text(encoding="utf-8"))

    run_id = str(args.run_id or config.get("run_id") or f"hrm_hermes_{int(time.time())}")
    ...
    dump_json(
        ...
        build_stage_manifest(
            ...
            "prepare_config",
            "completed",
            [str(base_trainer_config), str(config_path)],
            [str(resolved_cfg_path)],
            {"override_keys": len(trainer_overrides.keys())},
            notes=["Resolved HRM trainer config for Hermes-run launch"],
        ),
    )
    ...
    expected_outputs = [
        str(trainer_run_dir / "events.jsonl"),
        str(trainer_run_dir / "summary.json"),
        str(trainer_run_dir / "checkpoints"),
    ]
    dump_json(
        ...
        build_stage_manifest(
            ...
            notes=["HRM trainer launched under Hermes manifest wrapper"],
        ),
    )
