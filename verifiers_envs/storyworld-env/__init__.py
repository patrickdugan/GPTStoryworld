    ...
    for _ in range(num_runs):
        ...
        if best_end:
            ...
            final_samples.append(
                {
                    "ending_id": best_end["id"],
                    "state": {f"{char}.{prop}": float(val) for (char, prop), val in state.items()},
                }
            )
        else:
            dead_ends += 1
...
def _infer_morality_graded_properties(data: Dict[str, Any]) -> Tuple[List[str], List[str]]:
    profile = data.get("evaluation_profile", {}) if isinstance(data.get("evaluation_profile"), dict) else {}
    explicit_graded = profile.get("graded_properties") or data.get("graded_properties") or []
    explicit_context = profile.get("context_properties") or data.get("context_properties") or []
    authored = [str(p.get("property_name", p.get("id", "")) or "") for p in data.get("authored_properties", []) or []]
    authored = [p for p in authored if p]
    if explicit_graded:
        graded = [p for p in explicit_graded if p in authored]
    else:
        moral_tokens = {
            "duty",
            "mercy",
        ...
        for p in authored:
            ...
            if any(tok in low for tok in moral_tokens):
                graded.append(p)
    if explicit_context:
        context = [p for p in explicit_context if p in authored and p not in graded]
    else:
        context = [p for p in authored if p not in graded]
    return graded, context


def _sample_state_value(sample_state: Dict[str, float], prop: str) -> float:
    key = f"char_executor.{prop}"
    if key in sample_state:
        return float(sample_state[key])
    suffix = "." + prop
...
def _build_local_max_anchors(data: Dict[str, Any], graded_props: List[str]) -> Dict[str, Dict[str, Any]]:
    endings = detect_endings(data)
    all_props = [str(p.get("property_name", p.get("id", "")) or "") for p in data.get("authored_properties", []) or []]
    all_props = [p for p in all_props if p]
    if not endings or not graded_props:
        return {}

    base_state = {( "char_executor", p): 0.5 for p in all_props}
    anchors: Dict[str, Dict[str, Any]] = {}
    for end in endings:
        eid = str(end.get("id", "") or "")
        if not eid:
            continue
        desirability = end.get("desirability_script", 0.0)
        if isinstance(desirability, (int, float, bool)):
            # Constant desirability doesn't define a local preference basin.
            continue
        anchor: Dict[str, float] = {}
        weights: Dict[str, float] = {}
        for prop in graded_props:
            st_lo = dict(base_state)
            st_hi = dict(base_state)
            st_lo[("char_executor", prop)] = 0.1
            st_hi[("char_executor", prop)] = 0.9
            dlo = eval_script(desirability, st_lo)
            dhi = eval_script(desirability, st_hi)
            try:
                dlo_f = float(dlo)
            except Exception:
                dlo_f = 0.0
            ...
            if delta > 0.05:
                ...
                anchor[prop] = 0.5
            weights[prop] = max(0.1, min(2.5, abs(delta)))
        anchors[eid] = {"anchor": anchor, "weights": weights}
    return anchors


def compute_morality_manifold_metrics(data: Dict[str, Any], report: Dict[str, Any]) -> Dict[str, float]:
    graded_props, context_props = _infer_morality_graded_properties(data)
    samples = report.get("final_samples", []) or []
    if not graded_props or not samples:
        return {
            "graded_var_count": float(len(graded_props)),
            "context_var_count": float(len(context_props)),
            "local_max_proximity": 0.0,
            "context_flow_variability": 0.0,
            "moral_manifold_score": 0.0,
        }

    anchors = _build_local_max_anchors(data, graded_props)
    if not anchors:
        return {
            "graded_var_count": float(len(graded_props)),
            "context_var_count": float(len(context_props)),
    ...
    for s in samples:
        ...
        dist = 0.0
        for prop in graded_props:
            w = float(weights.get(prop, 0.1))
            actual = _sample_state_value(st, prop)
            target = float(anchor.get(prop, 0.5))
            den += w
            dist += w * min(1.0, abs(actual - target))
        if den <= 1e-9:
            continue
        prox_vals.append(max(0.0, 1.0 - dist / den))
    local_max_proximity = sum(prox_vals) / len(prox_vals) if prox_vals else 0.0

    def avg_std(props: List[str]) -> float:
        if not props:
            return 0.0
        stds: List[float] = []
        for prop in props:
            vals = [_sample_state_value((s.get("state") or {}), prop) for s in samples]
            if len(vals) < 2:
                stds.append(0.0)
                continue
    ...
    # Nongraded "flow" variables matter for route texture; include softly.
    context_std = avg_std(context_props)
    context_flow_variability = max(0.0, min(1.0, context_std / 0.15))
    moral_manifold_score = 0.82 * local_max_proximity + 0.18 * context_flow_variability

    return {
        "graded_var_count": float(len(graded_props)),
        "context_var_count": float(len(context_props)),
        "local_max_proximity": local_max_proximity,
        "context_flow_variability": context_flow_variability,
        "moral_manifold_score": moral_manifold_score,
    }


def benchmark_targets(profile: str = "default") -> Dict[str, float]:
    """Targets aligned to STORYWORLD_BALANCING.md (or profile overrides)."""
    base = {
        ...
        # Morality manifold (optional for default profile)
        "local_max_proximity_min": 0.0,
        "moral_manifold_score_min": 0.0,
    }
    ...
    if profile_key == "morality_open":
        ...
        tuned.update(
            {
                # Open-manifold morality worlds: broad reachable endings, no secret-path requirement.
                "dead_end_rate_max": 0.08,
                "max_ending_share_max": 0.35,
                "min_ending_share_min": 0.003,
                "late_block_min": 0.0,
                "late_block_max": 0.50,
                "secret_reachability_min": 0.0,
                "secret_gate_quality_min": 0.0,
                "gated_ratio_min": 0.0,
                "gated_ratio_max": 1.0,
                "major_turns_min": 0.0,
                "ending_entropy_min": 2.2,
                "ending_entropy_soft_min": 2.2,
                "secret_reachability_max": 1.0,
                "ending_effective_min": 7.0,
                "effects_per_reaction_min": 4.0,
                "reactions_per_option_min": 2.0,
                "options_per_encounter_min": 3.0,
                "desirability_vars_min": 1.5,
                "act2_gate_pct_min": 0.0,
                "act2_gate_vars_min": 0.0,
                "act3_gate_pct_min": 0.0,
                "act3_gate_vars_min": 0.0,
                "secret_metric_distance_min": 0.0,
                "text_length_compliance_min": 0.28,
                # Allow concise but still non-trivial script stacks.
                "effect_nonconstant_ratio_min": 0.99,
                "des_nonconstant_ratio_min": 0.99,
                "effect_complexity_min": 1.0,
                "des_complexity_min": 1.0,
                "effect_operator_variety_min": 1.0,
                "des_operator_variety_min": 1.0,
                "effect_operator_dominance_max": 1.0,
                "des_operator_dominance_max": 1.0,
                "option_visibility_nonconstant_ratio_min": 0.10,
                "option_visibility_complexity_min": 0.8,
                "option_performability_nonconstant_ratio_min": 0.10,
                "option_performability_complexity_min": 0.8,
                "encounter_acceptability_nonconstant_ratio_min": 0.20,
                "encounter_acceptability_complexity_min": 0.8,
                "encounter_desirability_nonconstant_ratio_min": 0.20,
                "encounter_desirability_complexity_min": 0.8,
                "encounter_theme_relevance_ratio_min": 0.62,
                "reaction_theme_relevance_ratio_min": 0.70,
                "encounter_theme_semantic_coherence_min": 0.22,
                "reaction_theme_semantic_coherence_min": 0.22,
                "local_max_proximity_min": 0.58,
                "moral_manifold_score_min": 0.54,
            }
        )
        return tuned
...
def evaluate_benchmark(data: Dict[str, Any], runs: int = 200, seed: int = 42) -> Dict[str, float]:
    ...
    manifold = compute_morality_manifold_metrics(data, report)
    return {
        ...
        "graded_var_count": manifold["graded_var_count"],
        "context_var_count": manifold["context_var_count"],
        "local_max_proximity": manifold["local_max_proximity"],
        "context_flow_variability": manifold["context_flow_variability"],
        "moral_manifold_score": manifold["moral_manifold_score"],
    }


def benchmark_pass(metrics: Dict[str, float], profile: str = "default") -> bool:
    targets = benchmark_targets(profile=profile)
    profile_key = (profile or "default").strip().lower()
    ...
    if profile_key == "morality_open":
        return (
            ...
            and metrics.get("local_max_proximity", 0.0) >= targets["local_max_proximity_min"]
            and metrics.get("moral_manifold_score", 0.0) >= targets["moral_manifold_score_min"]
        )
    return (
        metrics["dead_end_rate"] <= targets["dead_end_rate_max"]
        and metrics["max_ending_share"] <= targets["max_ending_share_max"]
