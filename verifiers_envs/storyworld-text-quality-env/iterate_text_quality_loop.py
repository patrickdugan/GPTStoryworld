...
from evaluate_text_quality import extract_samples, run_judge
...
def _collect_rewrite_targets(data: Dict[str, Any], judge: Dict[str, Any], max_items: int = 80) -> Dict[str, Any]:
    # Use richer targets so the writer can improve mechanics/characterization relevance.
    samples = extract_samples(data, max_encounters=10_000, max_reactions=100_000)
    encounter_map: Dict[str, Dict[str, Any]] = {str(e.get("id", "")): e for e in samples.get("encounters", []) or []}
    reaction_map: Dict[str, Dict[str, Any]] = {str(r.get("id", "")): r for r in samples.get("reactions", []) or []}

    failing = judge.get("failing_examples", []) or []
    enc_targets: List[Dict[str, Any]] = []
    rxn_targets: List[Dict[str, Any]] = []
    for item in failing:
        sid = str(item.get("id", ""))
...
def _request_rewrites(
    ...
    prompt = (
        ...
        "- Use provided mechanics context (spools, acceptability/desirability, variables/effects) to ground the prose.\n"
        "- If characters are involved in the target's scripts/effects, reflect characterization in the writing.\n"
    )
    payload = {
        ...
        "characters": [
            {"id": str(c.get("id", "")), "name": str(c.get("name", "")), "pronoun": str(c.get("pronoun", ""))}
